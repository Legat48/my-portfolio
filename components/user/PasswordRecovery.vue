<template>
  <div
    class="recovery"
    :class="{'recovery_modal' : !modal}"
  >
    <NoModalHeader v-if="!modal" class="recovery__header" :title="$t('settings.changePassword')">
      <template #svg>
    ...
    <label for="email" class="recovery__label">
      <input
        ref="email"
        v-model="email"
        ...
        required
      >
      <p class="recovery__label-info" :class="{'recovery__label-info_error' : mailError}">
        {{ $t('auth.emailValid') }}
      </p>
    </label>
    <button
      ref="submitButton"
      v-ripple
      :class="{'recovery__btn_disabled' :!valid}"
      class="recovery__btn btn"
      @click="submit"
    >
      <v-icon class="recovery__btn-icon" dense color="#FFF">
        mdi-account-edit-outline
      </v-icon>
      {{ $t('buttons.submit') }}
    </button>
    <p v-if="error" class="recovery__error">
      {{ error }}
    </p>
    <p v-if="message" class="recovery__message">
      {{ message }}
    </p>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateValid () {
      this.valid = !this.mailError && this.email
    },
    async submit () {
      this.valid = false
      this.error = ''
      this.message = ''
      try {
        await this.$store.dispatch('user/passwordRecovery', {
          email: this.email.toLowerCase(),
          portfolio: '1',
          language: this.$i18n.locale
        })
        this.message = this.$t('settings.passwordRecoveryLetter')
      } catch (e) {
        console.error(e)
        this.valid = true
